# Fistchain-compatible cross-chain atomic swapping
...
* Fistchain ([fistwallet](https://github.com/Fistchain/fistwallet))
...
1](https://github.com/Fistchain/atomicswap/projects/1) for the status of coins
...
how to use them safely, and an example swap between Fistchain and Bitcoin.
...
- Clone atomicswap somewhere outside `$GOPATH`:
  ```
  $ git clone https://github.com/Fistchain/atomicswap && cd atomicswap
  ```

- To install a single tool:
  ```
  $ cd cmd/fistatomicswap && go install
  ```

## Theory

A cross-chain swap is a trade between two users of different cryptocurrencies.
For example, one party may send Fistchain to a second party's Fistchain address, while
...
For simplicity, we assume the initiator wishes to trade Bitcoin for Fistchain with
the participant.  The initiator can also trade Fistchain for Bitcoin and the steps
...
the Fistchain blockchain and pays the intended Fistchain amount into the contract.
...
the participant's Fistchain contract, thereby revealing the secret to the
participant.  The secret is then extracted from the initiator's redeeming Fistchain
...
between Bitcoin and Fistchain, the two utilities `btcatomicswap` and
`fistatomicswap` are used.  Both tools must be used by both parties performing
the swap.
...
password while `fistatomicswap` does not.  Running a tool without any parameters
...
For fistatomicswap, this step prompts for the wallet passphrase.  For the
...
party A (the initiator) wishes to trade Bitcoin for Fistchain, party B (the
participant) must provide their Bitcoin address and the initiator must provide
the participant their Fistchain address.

_Party A runs:_
```
$ fistctl --testnet --wallet getnewaddress
...
by paying the intended Fistchain amount (1.0 in this example) into a Fistchain
...
_Party B runs:_
```
$ fistatomicswap --testnet participate TsfWDVTAcsLaHUhHnLLKkGnZuJz2vkmM6Vr 1.0 29c36b8dd380e0426bdc1d834e74a630bfd5d111
Passphrase:

Contract fee: 0.000251 FIST (0.00100400 FIST/kB)
Refund fee:   0.000301 FIST (0.00100669 FIST/kB)
...
B now informs A that the Fistchain contract transaction has been created and
...
1. The recipient address was the FIST address that was provided to B
2. The contract value is the expected amount of FIST to receive
3. The locktime was set to 24 hours in the future
4. The secret hash matches the value previously known

_Party A runs:_
```
$ fistatomicswap --testnet auditcontract 63a61429c36b8dd380e0426bdc1d834e74a630bfd5d1118876a9149ee19833332a04d2be97b5c99c970191221c070c6704e6dabb59b17576a914b0ec0640c89cf803b8fdbd6e0183c354f71748c46888ac 
Contract address:        TcZpybEVDVTuoE3TCBxW3ui12YEZWrw5ccS
Contract value:          1 FIST
...
from the Fistchain contract.  This step involves publishing a transaction which
reveals the secret to B, allowing B to withdraw from the Bitcoin contract.

_Party A runs:_
```
$ fistatomicswap --testnet redeem 63a61429c36b8dd380e0426bdc1d834e74a630bfd5d1118876a9149ee19833332a04d2be97b5c99c970191221c070c6704e6dabb59b17576a914b0ec0640c89cf803b8fdbd6e0183c354f71748c46888ac  3e0b064c97247732a3b345ce7b2a835d928623cb2871c26db4c2539a38e61a16
Passphrase:

Redeem fee: 0.000334 FIST (0.00100300 FIST/kB)
...
Now that A has withdrawn from the Fistchain contract and revealed the secret, B
must extract the secret from this redemption transaction.  B may watch a block
explorer to see when the Fistchain contract output was spent and look up the
redeeming transaction.

_Party B runs:_
```
$ fistatomicswap --testnet extractsecret  29c36b8dd380e0426bdc1d834e74a630bfd5d111
Secret: 3e0b064c97247732a3b345ce7b2a835d928623cb2871c26db4c2539a38e61a16
```
...
performed on the public Bitcoin and Fistchain testnet blockchains.  For reference,
...
| Fistchain contract created by B | [a51a7ebc178731016f897684e8e6fbbd65798a84d0a0bd78fe2b53b8384fd918](https://testnet.Fistchain.org/tx/a51a7ebc178731016f897684e8e6fbbd65798a84d0a0bd78fe2b53b8384fd918) |
| A's Fistchain redemption | [53c2e8bafb8fe36d54bbb1884141a39ea4da83db30bdf3c98ef420cdb332b0e7](https://testnet.Fistchain.org/tx/53c2e8bafb8fe36d54bbb1884141a39ea4da83db30bdf3c98ef420cdb332b0e7) |
...
{insight,testnet}.litecore.io, and the Fistchain block explorer on
{mainnet,testnet}.Fistchain.org, the API endpoint `/api/rawtx/<txhash>` can be used
...
| Fistchain contract created by B | https://testnet.Fistchain.org/api/rawtx/a51a7ebc178731016f897684e8e6fbbd65798a84d0a0bd78fe2b53b8384fd918 |
| A's Fistchain redemption | https://testnet.Fistchain.org/api/rawtx/53c2e8bafb8fe36d54bbb1884141a39ea4da83db30bdf3c98ef420cdb332b0e7 |
...
## First mainnet FIST-LTC atomic swap

| Description | Link to raw transaction |
| - | - |
| Fistchain contract created by A | [fdd72f5841414a9c8b4a188a98a4d484df98f84e1c120e1ed59a66e51e8ae90c](https://mainnet.Fistchain.org/tx/fdd72f5841414a9c8b4a188a98a4d484df98f84e1c120e1ed59a66e51e8ae90c) |
...
| B's Fistchain redemption | [49245425967b7e39c1eb27d261c7fe972675cccacff19ae9cc21f434ccddd986](https://mainnet.Fistchain.org/tx/49245425967b7e39c1eb27d261c7fe972675cccacff19ae9cc21f434ccddd986) |
## License
