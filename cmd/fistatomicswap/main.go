// Copyright (c) 2017 The Fistchain developers
...
import (
	...
	"github.com/Fistchain/fistd/chaincfg"
	"github.com/Fistchain/fistd/chaincfg/chainhash"
	"github.com/Fistchain/fistd/fistec"
	"github.com/Fistchain/fistd/fistutil"
	"github.com/Fistchain/fistd/txscript"
	"github.com/Fistchain/fistd/wire"
	pb "github.com/Fistchain/fistwallet/rpc/walletrpc"
	"github.com/Fistchain/fistwallet/wallet/txrules"
...
var (
	...
	connectFlag = flagset.String("s", "localhost", "host[:port] of fistwallet gRPC server")
	certFlag    = flagset.String("c", filepath.Join(fistutil.AppDataDir("fistwallet", false), "rpc.cert"), "fistwallet RPC certificate path")
...
// Fistchain transactions for these swaps.  A second tool should be used for the
...
//   cp2 redeems fist with S
//
// Scenerio 2:
//   cp1 initiates (btc)
//   cp2 participates with cp1 H(S) (fist)
//   cp1 redeems fist revealing S
//     - must verify H(S) in contract is hash of known secret
//   cp2 redeems btc with S

func init() {
	flagset.Usage = func() {
		fmt.Println("Usage: fistatomicswap [flags] cmd [cmd args]")
		fmt.Println()
...
type initiateCmd struct {
	cp2Addr fistutil.Address
	amount  fistutil.Amount
}

type participateCmd struct {
	cp1Addr    fistutil.Address
	amount     fistutil.Amount
	secretHash []byte
}
...
func run() (err error, showUsage bool) {
	...
	switch args[0] {
	case "initiate":
		cp2Addr, err := fistutil.DecodeAddress(args[1])
		if err != nil {
		...
		_, ok := cp2Addr.(*fistutil.AddressPubKeyHash)
		if !ok {
		...
		amount, err := fistutil.NewAmount(amountF64)
		if err != nil {
			return err, true
		}
	...
	case "participate":
		cp1Addr, err := fistutil.DecodeAddress(args[1])
		if err != nil {
		...
		_, ok := cp1Addr.(*fistutil.AddressPubKeyHash)
		if !ok {
...
type builtContract struct {
	contract       []byte
	contractP2SH   fistutil.Address
	contractTxHash *chainhash.Hash
	contractTx     *wire.MsgTx
	contractFee    fistutil.Amount
	refundTx       *wire.MsgTx
	refundFee      fistutil.Amount
}
...
func buildContract(ctx context.Context, c pb.WalletServiceClient, args *contractArgs,
	...
	refundAddr, err := fistutil.DecodeAddress(nar.Address)
	if err != nil {
		return nil, err
	}
	if _, ok := refundAddr.(*fistutil.AddressPubKeyHash); !ok {
	...
	contractP2SH, err := fistutil.NewAddressScriptHash(contract, chainParams)
	if err != nil {
		return nil, err
	}
	...
	contractFee := fistutil.Amount(ctr.TotalPreviousOutputAmount - ctr.TotalOutputAmount)
...
func buildRefund(ctx context.Context, c pb.WalletServiceClient, contract []byte, contractTx *wire.MsgTx,
	feePerKb fistutil.Amount, passphrase []byte) (
	refundTx *wire.MsgTx, refundFee fistutil.Amount, err error) {
	...
	refundAddress, err := fistutil.DecodeAddress(nar.Address)
	if err != nil {
		return nil, 0, err
	}
	...
	refundAddr, err := fistutil.NewAddressPubKeyHash(pushes.RefundHash160[:], chainParams,
		fistec.STEcdsaSecp256k1)
	if err != nil {
		return nil, 0, err
	}
	...
	if txrules.IsDustOutput(refundTx.TxOut[0], feePerKb) {
		return nil, 0, fmt.Errorf("refund output value of %v is dust", fistutil.Amount(refundTx.TxOut[0].Value))
	}
...
func calcFeePerKb(absoluteFee fistutil.Amount, serializeSize int) float64 {
...
func (cmd *initiateCmd) runCommand(ctx context.Context, c pb.WalletServiceClient) error {
	...
	fmt.Printf("Contract fee: %v (%0.8f FIST/kB)\n", b.contractFee, contractFeePerKb)
	fmt.Printf("Refund fee:   %v (%0.8f FIST/kB)\n\n", b.refundFee, refundFeePerKb)
...
func (cmd *redeemCmd) runCommand(ctx context.Context, c pb.WalletServiceClient) error {
	...
	recipientAddr, err := fistutil.NewAddressPubKeyHash(pushes.RecipientHash160[:],
		chainParams, fistec.STEcdsaSecp256k1)
	if err != nil {
		return err
	}
	contractHash := fistutil.Hash160(cmd.contract)
	contractOut := -1
	...
	addr, err := fistutil.DecodeAddress(nar.Address)
	if err != nil {
		return err
	}
	...
	if txrules.IsDustOutput(redeemTx.TxOut[0], feePerKb) {
		return fmt.Errorf("redeem output value of %v is dust", fistutil.Amount(redeemTx.TxOut[0].Value))
	}
	...
	redeemTx.Serialize(&buf)
	fmt.Printf("\n")
	fmt.Printf("Redeem fee: %v (%0.8f FIST/kB)\n\n", fee, redeemFeePerKb)
...
func (cmd *auditContractCmd) runOfflineCommand() error {
	contractHash160 := fistutil.Hash160(cmd.contract)
	contractOut := -1
	for i, out := range cmd.contractTx.TxOut {
		...
		if bytes.Equal(addrs[0].(*fistutil.AddressScriptHash).Hash160()[:], contractHash160) {
			contractOut = i
			break
		}
	}
	...
	contractAddr, err := fistutil.NewAddressScriptHash(cmd.contract, chainParams)
	if err != nil {
		return err
	}
	recipientAddr, err := fistutil.NewAddressPubKeyHash(pushes.RecipientHash160[:],
	...
	refundAddr, err := fistutil.NewAddressPubKeyHash(pushes.RefundHash160[:],
		chainParams, fistec.STEcdsaSecp256k1)
	if err != nil {
		return err
	}
	...
	fmt.Printf("Contract value:          %v\n", fistutil.Amount(cmd.contractTx.TxOut[contractOut].Value))
